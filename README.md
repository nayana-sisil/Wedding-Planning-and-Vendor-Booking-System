...
<head>
    ...
    <style>
        ...
        :root {
            ...
            --secondary-color: #ffab00; /* Kept for minor accents */
            --error-color: #c62828; /* Darker red for errors */
            --success-color: #388e3c; /* Kept for success states */
            --text-color: #333; /* Dark text for readability */
            --light-gray: #ffffff; /* White background */
            --medium-gray: #e0e0e0; /* Light gray for borders */
            --dark-gray: #757575; /* Dark gray for secondary text */
        ...
        * {
        ...
        .admin-header {
            grid-area: header;
            background-color: var(--primary-color); /* Red header */
            color: var(--white);
            display: flex;
            justify-content: space-between;
            align-items: center;
        ...
        #logout-btn {
            background-color: var(--primary-light); /* Lighter red */
            color: var(--white);
            border: none;
            padding: 0.5rem 1rem;
            border-radius: var(--border-radius);
        ...
        .admin-sidebar {
            grid-area: sidebar;
            background-color: var(--white); /* White sidebar */
            box-shadow: var(--shadow);
            padding: 1.5rem 0;
        }
        ...
        .admin-sidebar a:hover,
        .admin-sidebar li.active a {
            background-color: var(--primary-light); /* Lighter red for active/hover */
            color: var(--white);
        }
        ...
        .admin-content {
            ...
            padding: 2rem;
            overflow-y: auto;
            background-color: var(--white); /* White content background */
        }
        ...
        .stat-card p {
            font-size: 2rem;
            font-weight: 600;
            color: var(--primary-color); /* Red for stats */
        }
        ...
        #admins-table tr:hover {
            background-color: #fff5f5; /* Very light red for hover */
        }
        ...
        .btn-danger {
            background-color: var(--error-color); /* Darker red */
            color: var(--white);
        }
        ...
        .status-inactive {
            background-color: #ffebee; /* Light red for inactive */
            color: var(--error-color);
        }
        ...
        <section id="admins-section" class="content-section">
            ...
            <div class="admin-table-container">
                <table id="admins-table">
                    ...
                    <!-- Data loaded dynamically -->
                    <tr class="loading-row">
                        <td colspan="6">Loading admin data...</td>
                    </tr>
                    </tbody>
                </table>
                <div class="table-pagination">
...
<script>
    ...
    class Admin extends User {
        ...
        constructor(id, username, email, permissions = [], isActive = true, isSuperAdmin = false) {
        ...
        isSuperAdmin() { return this.#isSuperAdmin; }
        getRole() { return 'Admin'; }

       
        setPermissions(permissions) { this.#permissions = permissions; }
        setActive(status) { this.#isActive = status; }
        setSuperAdmin(status) { this.#isSuperAdmin = status; }

        
        manageAdmin(adminData) {
            if (!this.#isSuperAdmin) {
                throw new Error('Only super admins can manage other admins');
            }
            return `Managing admin: ${adminData.username}`;
        }
    }
    ...
    class AdminManager {
        ...
        loadFromStorage() {
            ...
            if (adminsData) {
                const parsed = JSON.parse(adminsData);
                this.#admins = parsed.map(data => 
                    new Admin(
                        data.id,
                        data.username,
                        data.email,
        ...
        createAdmin(adminData) {
            const newAdmin = new Admin(
                ...
                adminData.isActive === 'active',
                adminData.isSuperAdmin || false
            );
            this.#admins.push(newAdmin);
            this.logActivity(newAdmin.getId(), `Created admin ${adminData.username}`);
            this.saveToStorage();
            return newAdmin;
        }
        ...
        updateAdmin(adminId, adminData) {
            const admin = this.#admins.find(a => a.getId() === adminId);
            if (!admin) throw new Error('Admin not found');
            admin.setUsername(adminData.username);
            admin.setEmail(adminData.email);
            admin.setPermissions(adminData.permissions);
            admin.setActive(adminData.isActive === 'active');
            this.logActivity(adminId, `Updated admin ${adminData.username}`);
            this.saveToStorage();
            return admin;
        }

        deleteAdmin(adminId) {
            const index = this.#admins.findIndex(a => a.getId() === adminId);
            if (index === -1) throw new Error('Admin not found');
            const [admin] = this.#admins.splice(index, 1);
            this.logActivity(adminId, `Deleted admin ${admin.getUsername()}`);
            this.saveToStorage();
            return admin;
        }

        logActivity(adminId, action) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const quickAddAdmin = document.getElementById('quick-add-admin');
        const quickViewLogs = document.getElementById('quick-view-logs');
        ...
        let currentAdmin = new Admin(
            ...
            ['manage_users', 'manage_content', 'view_reports', 'system_settings'],
            true,
            true
        );
        ...
        logoutBtn.addEventListener('click', logout);
        addAdminBtn.addEventListener('click', showAddAdminForm);
        quickAddAdmin.addEventListener('click', showAddAdminForm);
        quickViewLogs.addEventListener('click', () => showSection('logs'));
        closeModal.addEventListener('click', closeAdminModal);
        adminForm.addEventListener('submit', handleAdminSubmit);
        applyFiltersBtn.addEventListener('click', applyLogFilters);
        resetFiltersBtn.addEventListener('click', resetLogFilters);
        ...
        function renderAdminsTable() {
            ...
            const admins = adminManager.readAdmins();
            if (admins.length === 0) {
            ...
            admins.forEach(admin => {
                const row = document.createElement('tr');
                row.innerHTML = `
                    <td>${admin.getId().substring(0, 8)}...</td>
                    <td>${admin.getUsername()}</td>
                    <td>${admin.getEmail()}</td>
                    <td>${admin.getPermissions().join(', ')}</td>
                    <td><span class="status-badge ${admin.isActive() ? 'status-active' : 'status-inactive'}">
                        ${admin.isActive() ? 'Active' : 'Inactive'}
                    </span></td>
                    <td>
                        <button class="btn btn-secondary edit-admin" data-id="${admin.getId()}">Edit</button>
                        ${currentAdmin.isSuperAdmin() ? `<button class="btn btn-danger delete-admin" data-id="${admin.getId()}">Delete</button>` : ''}
                    </td>
                `;
                tableBody.appendChild(row);
            });
        ...
        function renderActivityLogs(filteredLogs = adminManager.getActivityLogs()) {
            const logsContainer = document.querySelector('.logs-list');
            logsContainer.innerHTML = '';
            ...
            filteredLogs.forEach(log => {
                ...
                logItem.innerHTML = `
                    <div class="log-meta">
                        <span>${new Date(log.timestamp).toLocaleString()}</span>
                        <span>Admin: ${log.adminId.substring(0, 8)}...</span>
                    </div>
                    <p>${log.action}</p>
                `;
                logsContainer.appendChild(logItem);
        ...
        function populateAdminFilter() {
            ...
            const adminIds = [...new Set(adminManager.getActivityLogs().map(log => log.adminId))];
            adminIds.forEach(id => {
                const option = document.createElement('option');
                option.value = id;
                option.textContent = id.substring(0, 8) + '...';
        ...
        function applyLogFilters() {
            ...
            let filteredLogs = [...adminManager.getActivityLogs()];
        ...
        function resetLogFilters() {
            document.getElementById('log-filter-admin').value = '';
        ...
        function showEditAdminForm(adminId) {
            const admin = adminManager.readAdmins().find(a => a.getId() === adminId);
            if (!admin) return;

            document.getElementById('modal-title').textContent = 'Edit Admin';
            document.getElementById('admin-id').value = admin.getId();
            document.getElementById('username').value = admin.getUsername();
            document.getElementById('email').value = admin.getEmail();
            document.getElementById('admin-status').value = admin.isActive() ? 'active' : 'inactive';
            ...
            permissionCheckboxes.forEach(checkbox => {
                checkbox.checked = admin.getPermissions().includes(checkbox.value);
            });
        ...
        function handleAdminSubmit(e) {
            ...
            const adminData = {
                ...
                email: document.getElementById('email').value,
                permissions,
                isActive: document.getElementById('admin-status').value
            };

            try {
                if (adminId) {
                    adminManager.updateAdmin(adminId, adminData);
        ...
        function updateStats() {
            const admins = adminManager.readAdmins();
            document.getElementById('total-admins').textContent = admins.length;
            document.getElementById('active-sessions').textContent = admins.filter(a => a.isActive()).length;
            const today = new Date().toDateString();
            document.getElementById('today-activities').textContent = adminManager.getActivityLogs()
                .filter(log => new Date(log.timestamp).toDateString() === today).length;
        }

        function updateUI() {
            document.getElementById('current-admin').textContent = `Welcome, ${currentAdmin.getUsername()}`;
            if (!currentAdmin.isSuperAdmin()) {
                addAdminBtn.style.display = 'none';
                quickAddAdmin.style.display = 'none';
            }
        }

        function logout() {
            adminManager.logActivity(currentAdmin.getId(), 'Logged out');
            window.location.href = '/login.html';
        }
